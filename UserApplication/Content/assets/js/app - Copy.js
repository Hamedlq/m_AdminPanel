...
$.ajax({
dataType: "json",
url: "http://localhost:50226/api/geo/getagencies",
...
$.getJSON('http://localhost:50226/getUsers', { get_param: 'value' })
.done(function(data) {
    $.each(data, function(index, element) {
		$("#user-list tbody").append('<tr class="feature-row"><td style="vertical-align: middle;"><img width="16" height="18" src="assets/img/museum.png"></td><td class="feature-name">' + element.Name + '</td><td style="vertical-align: middle;"><i class="fa fa-chevron-right pull-right"></i></td></tr>');
    });
})
.fail(function(error) { alert('getJSON request failed! '+error); })

$('#userPic,#carCard,#carCardBack,#bankCard').fileinput({
	language: 'fa',
	showUpload: false,
	autoReplace: true,
	showCaption: false,
...
$("#personalInfoForm").submit(function (event) {
    ...
    $.ajax({
        dataType: "json",
        type: 'POST',
        url: "/user/RegisterAgencyDriver",
        data:$this.serializeArray(),
        success: function (datastr) {
            switch (data.status) {
                case "OK":
                    $("#mobile").attr('disabled', true);
                    activeNextTab('userinfo');
                    break;
                case "Error":
                    $(".userModal-alert-wrap").append('<div class="alert alert-danger"><strong>خطا : </strong>' + datastr.errorMessage + '</div>');
                    break;
                case "UnAuthorized":
            ...
            $this.find("button[type=submit]").text("ادامه...");
        }
    }).error(function(error) {
        $(".userModal-alert-wrap").append('<div class="alert alert-danger"><strong>خطا : </strong>خطا در ارتباط</div>');
        $this.find("button[type=submit]").text("ادامه...");
    });
